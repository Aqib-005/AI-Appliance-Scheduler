...
for i in range(len(future_data)):
    ...
    pred = pred_scaled * y_train_std + y_train_mean
    lstm_future_pred.append(pred[0][0])
    
    # Update Lag_Price for the next timestep
    if i < len(future_data) - 1:
        future_data.at[future_data.index[i+1], 'Lag_Price'] = pred[0][0]

# Create a DataFrame for the predictions
future_predictions_df = pd.DataFrame({
    'Start date/time': future_dates,
    'Predicted Price [Euro/MWh]': lstm_future_pred
})
