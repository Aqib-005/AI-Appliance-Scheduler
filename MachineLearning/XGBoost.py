...
# Rename columns for consistency (adjust these based on your actual CSV column names)
...
# Convert datetime with specific format (assuming 'DD/MM/YYYY HH:MM')
...
# Future Forecasting (abbreviated for brevity; apply similar fixes as above)
...
# Add similar imputation for future data
...
last_historical_data = hist_data.tail(48)  # Last 48 hours for initialization
predictions = []

# Initialize histories with numeric lists
...
for idx, row in future_df.iterrows():
    # Extract current row features, ensuring numeric
    ...
    Day = dt.day
    Hour = dt.hour
    DayOfWeek = dt.dayofweek  # Consistent with day_map
    ...
    # Use previous predictions or historical values for lag prices
    Lag_Price_1h = predictions[-1] if predictions else last_historical_data['Price'].iloc[-1]
    Lag_Price_6h = predictions[-6] if len(predictions) >= 6 else last_historical_data['Price'].iloc[-6]
    Lag_Price_24h = predictions[-24] if len(predictions) >= 24 else last_historical_data['Price'].iloc[-24]
